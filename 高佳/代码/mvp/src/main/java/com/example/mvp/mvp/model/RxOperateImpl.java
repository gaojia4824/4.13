...
public class RxOperateImpl {
    ...
    public <T> void requestFormData(String url, Map<String, T> headers, Map<String, T> params, IDataCallBack<T> dataCallBack) {
        ...
        else if (params == null || params.size() == 0)  //参数为空但是请求头不为空
    ...
    //多文件+多参数上传
    public <T> void uploadStrFiles(String url, String param, IDataCallBack<T> dataCallBack, File... files) {
        if (files != null && files.length > 0) {
            if (param == null || param.length() == 0) {
                ...
                for (File file : files) {
                    ...
                    RxSchedulersOperator.retryWhenOperator(mApiService.uploadStrFiles(url, requestBody, part))
                            .subscribe(getObserver(dataCallBack));
                }
            }
        }
